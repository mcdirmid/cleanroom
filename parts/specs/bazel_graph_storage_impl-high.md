...
terms (from sandbox): blame, blame target, template
...
- The sandbox configuration constructed for a node grants read access to the node's declared source and its deps' declared sources, and write access to the node's own declared source (including its silent sources). Silent deps' declared sources are not readable.
