...
- [ ] Singletons registered in `LifecycleRegistry` are resolved within an active lifecycle scope using `with enter_phase("agent_session", registry=self.registry) as scope:` and `scope.get_singleton(TargetClass)` (calling non-existent methods like `registry.resolve` is prohibited)
