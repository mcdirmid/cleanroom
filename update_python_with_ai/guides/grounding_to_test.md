...
Initial test skeletons with valid imports, a starter test case, and cataloged untested requirements are auto-generated by the test linter (`test_lint.py`) directly from the grounding specification. Target classes share the exact specification class name (`from <target_impl> import <TargetClass>` or `from lib.<target_impl> import <TargetClass>`; the test linter auto-normalizes module imports to full package paths); mocking the target class or concrete data types is prohibited, while collaborator interface protocols are mocked. Test editing is strictly modular, fine-grained, and targeted, using `replace_file_content` to replace small, contiguous code blocks strictly one single test method or fixture at a time (at most 30–50 lines of code); whole-file rewrites, multi-class replacements, and monolithic multi-test replacements in a single call are prohibited; initial skeletons and untested requirements are evolved one test method at a time across multiple small edits. Calling `check_file(path=...)` after each edit verifies syntax and type correctness before making further modifications. The search tool is never installed.
