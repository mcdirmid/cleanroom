...
- [ ] Type-level imports come from the immediate LLS closure — the `low/<name>.md` files the dependency comment lists; a type-level dependency outside it is a traceability violation, recorded rather than imported or defined
...
- [ ] An `# Implementation LLS:` heading appears only in a `<name>_impl.md` or `<name>_asm.md` file (virtual name `low/<name>_impl.md` or `low/<name>_asm.md`); an assembly class in an assembly LLS must end with the `Asm` suffix (`class FooAsm`)
