...
- [ ] When all tests pass, the QA log must be completely empty (0 bytes, delete all lines including headers) before calling advance
...
- [ ] Keep blame feedback relatively high-level and grounded in specification concepts: state the failing test, the root cause in terms of the LLS contract or type definition that was misunderstood, and the expected behavior; allow the receiving node to resolve the issue from its own specification
- [ ] Never dictate code-level fixes, exact Python syntax patches, variable reassignments, or code snippets in blame feedback; avoid contaminating the test node with library implementation details
- [ ] When all tests in a test class fail uniformly with missing data or empty state right after construction, inspect the test's `setUp()` or fixture helper functions first before diagnosing deep implementation logic
- [ ] Check mock target scopes: standard library mocks must be patched on `lib.<module_under_test>.<symbol>`, not globally on stdlib modules
- [ ] Check test fixture formatting against LLS requirements: verify that test fixture generators produce the exact structural delimiters (headings, fields) required by the contract
