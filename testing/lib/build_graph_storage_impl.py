...
Provides the build_graph_storage implementation that fulfills the
build_graph_storage (and hence dag_storage) contract: Bazel-workspace-backed
...
# text-format file named .testing.textproto, serialized from the
# `update_with_ai` message type defined by the testing.proto schema.
...
HARNESS_FILE = ".testing.textproto"
...
def _parse_textproto(text: str) -> Dict[str, Any]:
    ...
    The parser handles the grammar used by the testing.proto schema:
...
def _serialize_textproto(data: Dict[str, Any]) -> str:
    ...
    The output matches the testing.proto schema: a repeated `nodes`
...
class BaseBuildGraphStorageImpl(BuildGraphStorage):
    """
    Implementation of the build_graph_storage interface.
    ...
    def resolve_node_definition(self, node_id: NodeId) -> NodeDefinition:
        ...
        Operation Implemented: build_graph_storage.resolve_node_definition
    ...
    def resolve_package_directory(self, node_id: NodeId) -> PackageDirectory:
        ...
        Operation Implemented: build_graph_storage.resolve_package_directory
...
def _build_verify_callback(verify_cmd: str) -> Optional[VerificationCallback]:
    ...
    advance() — see specs/high/sandbox.md / specs/low/sandbox.md.
    """
    if not verify_cmd:
        return None
...
class BuildGraphStorageFileImpl(BaseBuildGraphStorageImpl):
    ...
    def _load_manifests(self) -> None:
        ...
        # build_graph_storage_impl contract). The dependency's package
