...
This module provides the runtime representation of a build node
(BuildNodeImpl) and the manifest loader (BuildNodeLoaderImpl).
"""
...
from .build_node_loader import BuildNode, BuildNodeLoader
...
class BuildNodeImpl(BuildNode):
    """
    Runtime representation of a testing.

    Extends the interface's BuildNode data-class Protocol (which itself extends
    ...
    _dependency_nodes: List["BuildNodeImpl"] = field(default_factory=list)  # Loaded at runtime
    ...
    def _load_dependencies(self, load_node_fn: Callable) -> List["BuildNodeImpl"]:
...
class BuildNodeLoaderImpl(BuildNodeLoader):
    ...
    def __init__(self) -> None:
        self._node_cache: Dict[str, BuildNodeImpl] = {}
    ...
    def load_node(self, label: str) -> Optional[BuildNode]:
        ...
        Returns:
            BuildNodeImpl instance or None if not found. Cached instances are
    ...
    def load_graph(self, root_label: str) -> Dict[str, BuildNode]:
        ...
        Returns:
            Dict mapping labels to BuildNode instances (root and transitive
