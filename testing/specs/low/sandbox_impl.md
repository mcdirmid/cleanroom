...
- Forms the `TerminateAgentWithSuccess` result using `dag_clean_logic` result types:
  ...
  - `blame` (valid pairs) — resolves each pair's target (a blameable artifact's virtual name) through the configured `blame_targets` mapping to the owning node's `NodeId`, then carries `FeedbackResult` whose messages convert each resolved `(target, feedback)` pair into a `(target, NodeMessage)` pair — the target as the `NodeId`, the feedback as a `NodeMessage` with kind `feedback` and text the feedback (per `dag_clean_logic`'s `FeedbackResult.messages` type); each pair is one (target, feedback) message
