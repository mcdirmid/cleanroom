"""
Tests for lib/build_agent_config_impl.py (BuildAgentConfigImpl).

Behavioral contract (see specs/low/build_agent_config_impl.md):
...
from testing.lib.build_agent_config import (
    AgentConfig,
    BuildAgentConfig,
    DEFAULT_CONFIG_TARGET,
)
from testing.lib.build_agent_config_impl import (
    ApiKeyNotFoundError,
...
class TestBuildAgentConfigImpl(unittest.TestCase):
    """Shared fixture for the impl-class tests."""

    def setUp(self) -> None:
        self._tmp = tempfile.mkdtemp(prefix="build_agent_config_test_")
        self._root = Path(self._tmp)
        self.impl = BuildAgentConfigImpl()

    def tearDown(self) -> None:
        shutil.rmtree(self._tmp, ignore_errors=True)

    def test_implements_build_agent_config_protocol(self) -> None:
        """The impl exposes the protocol's operation (structural conformance)."""
        def _use_as_protocol(p: BuildAgentConfig) -> BuildAgentConfig:
...
class TestParseTarget(TestBuildAgentConfigImpl):
...
class TestFindAndLoad(TestBuildAgentConfigImpl):
    ...
    def test_load_config_honors_session_start_reads_gate(self) -> None:
        """A generated module with session_start_reads disabled loads as False
        (the run-level gate is carried by the agent configuration)."""
        pkg_dir = self._root / "bazel-bin" / PACKAGE
        pkg_dir.mkdir(parents=True, exist_ok=True)
        (pkg_dir / (NAME + "_config.py")).write_text(
        ...
        config = self.impl.load_config("//agent_configs:default", workspace_root=str(self._root))
        self.assertFalse(config.session_start_reads)


class TestApiKeyResolution(TestBuildAgentConfigImpl):
...
class TestConfigTargetResolution(TestBuildAgentConfigImpl):
...
class TestBuildAgentLoopConfig(TestBuildAgentConfigImpl):
