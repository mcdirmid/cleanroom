...
from testing.lib.build_graph_storage import (
    GraphConfig,
    NodeDefinition,
    NodeId,
    PackageDirectory,
)
from testing.lib.build_graph_storage_impl import (
    BaseBuildGraphStorageImpl,
    BuildGraphStorageFileImpl,
)
from testing.lib.dag_storage import NodeMessage, MessageKind, PendingMessages
...
class TestMessageFileOperations(unittest.TestCase):
    ...
    def test_get_pending_messages_missing_file_returns_empty(self):
        """LLS: a missing .testing.textproto reads as empty and is not created."""
    ...
    def test_harness_file_is_update_with_ai_textproto_in_package_dir(self):
        """LLS: a single file named .testing.textproto in the node's package directory."""
    ...
    def test_single_messages_file_per_package_directory(self):
        """LLS: one .testing.textproto per package directory holds all nodes in it."""
    ...
    def test_different_packages_have_separate_message_files(self):
        """LLS: each package directory holds its own .testing.textproto."""
        self.graph.add_messages("node_a", [msg("a")])
        self.graph.add_messages("node_b", [msg("b")])
    ...
    def test_write_uses_temp_file_and_atomic_replace(self):
        """LLS: writes go to a temporary file, then atomically replaced onto .testing.textproto."""
        messages_file = self.harness_file("node_a")
        calls: List[tuple] = []
        ...
        with mock.patch(
            "testing.lib.build_graph_storage_impl.os.replace", side_effect=_recording_replace
        ):
            self.graph.add_messages("node_a", [msg("msg")])
    ...
    def test_write_failure_before_replace_leaves_previous_file_unchanged(self):
        ...
        with mock.patch(
            "testing.lib.build_graph_storage_impl.os.replace",
