...
The access gate resolves the active session scope for the specified conversation identifier. If no session is registered for the conversation identifier, validating access produces a denied access decision explaining that no active session exists.
...
- For file write tools matching `replace_file_content` or `write_to_file`, file write permissions are evaluated for the normalized path, producing an allowed access decision when the operation succeeds, or a denied access decision carrying the diagnostic content when denied.

- For file read tools matching `view_file`, file read permissions are evaluated for the normalized path, producing an allowed access decision when the operation succeeds, or a denied access decision carrying the diagnostic content when denied.
...
Filtering directory listings sanitizes entries inside the activated session scope. Candidate child file entries are tested against file read permissions, retaining entries that are readable and omitting entries that violate role blindness or target boundaries. If no session scope is registered for the conversation identifier, directory filtering returns an empty sequence of entries to fail closed.
