...
class DagRunner(dag_runner.DagRunner):
    """
PURPOSE:
...
- [DagRunner] A dag runner executes a cleaning pass over an acyclic subgraph rooted at a target node in dag storage.
- [DagRunner] A dag runner produces a build result upon pass completion.
...
INHERITED_REQUIREMENTS:
- [DagRunner] A dag runner marks a target node dirty by injecting a change message into its pending messages in dag storage.
...
INHERITED_REQUIREMENTS:
- [DagRunner] A dag runner injects a caller-supplied feedback message into a target node.
...
INHERITED_REQUIREMENTS:
- [DagRunner] A dag runner broadcasts a caller-supplied change message from a node to all of its reverse dependencies.
