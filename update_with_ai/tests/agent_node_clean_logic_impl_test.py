...
class TestIsDirty(unittest.TestCase):
    ...
    def test_not_dirty_when_empty_template_configured(self):
        """A writable file with an empty template is not treated as dirty when
        missing on disk or when empty on disk."""
        with tempfile.TemporaryDirectory() as tmp:
            out_path = os.path.join(tmp, "out.txt")
            node_def = _make_node_def(
                writable_paths=["out.txt"],
            ...
            # Also not dirty when existing as empty file
            with open(out_path, "w") as f:
                f.write("")
            self.assertFalse(impl.is_dirty("a", []))
            self.assertEqual(graph.get_pending_messages("a"), [])
