"""
tests/build_node_loader_impl_test.py

Unit tests for lib/build_node_loader_impl.py, asserting the implementation
LLS (specs/low/build_node_loader_impl.md) contract:

- Manifest paths derive deterministically from labels: //pkg:target ->
  pkg/target_manifest.json in runfiles (LLS invariant).
- load_node builds a BuildNodeImpl with all manifest fields, returns None for
...
- BuildNodeImpl tool-provider operations dynamically load provider modules via
...
from update_with_ai.lib.build_node_loader import BuildNode
from update_with_ai.lib.build_node_loader_impl import (
    BuildNodeImpl,
    BuildNodeLoaderImpl,
...
class _TempRunfilesTestCase(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self._runfiles_patcher = patch(
            "update_with_ai.lib.build_node_loader_impl._get_runfiles_path",
    ...
    def load_impl(self, label: str) -> BuildNodeImpl:
        """load_node(label) asserted to return a BuildNodeImpl (contract)."""
        node = self.loader.load_node(label)
        self.assertIsInstance(node, BuildNodeImpl)
        assert isinstance(node, BuildNodeImpl)
        return node


class TestLoadNode(_TempRunfilesTestCase):
    """load_node: manifests -> BuildNodeImpl, caching by label."""

    def test_load_node_returns_node_with_all_manifest_fields(self):
        """load_node builds a BuildNodeImpl carrying every manifest field."""
        self.write_manifest(
            "pkg",
            "target",
            "//pkg:target",
        ...
        self.assertIsInstance(node, BuildNode)
...
class TestBuildNodeImplToolProviders(unittest.TestCase):
    """BuildNodeImpl tool-provider operations (importlib loading, lazy cache)."""
    ...
    def make_node(self, tools: List[str]) -> BuildNodeImpl:
...
class TestRunPrompt(unittest.TestCase):
    ...
    def make_node(self) -> BuildNodeImpl:
        return BuildNodeImpl(
            label="//pkg:target",
            prompt="test prompt",
            tools=[],
