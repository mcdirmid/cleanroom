...
- For each node with `star_deps`, computes the star closure from the loaded manifests: every node reachable from a star dep through `star_deps` (never `deps` or `silent_deps`), traversed once each. Each node in the closure contributes its `srcs` to the node's readable set exactly like a direct dep; a star dep whose manifest is not loaded contributes nothing.
