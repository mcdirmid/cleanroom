...
- For each manifest file, constructs a `NodeDefinition` with `prompt` from the manifest and `sandbox_config` populated from `src`, `template`, `silent_srcs`, `deps`, `silent_deps`, `feedback_deps`, and `verify` fields (see `_build_sandbox_config` helper).
