...
terms (owned): run, termination value, conversation, conversation message
...
- Termination is observable dataflow: the signal produced by tool execution exits via the run result unchanged, paired with the conversation history.
...
- Defaults when omitted: an iteration limit of ten, a sampling temperature of 0.0, and a request timeout of 60 seconds.
...
- Signals failure, leaving state unchanged, when the run fails, the language model service fails, the response is malformed, or tool execution raises an exception.
- Signals failure, leaving state unchanged, when the run exceeds the configured maximum number of loop iterations.
- Maintains chronological conversation order and processes tool results per tool_provider semantics.
