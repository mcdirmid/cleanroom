...
- `feedback_pending` set and first advance without a change (no net-changed files, no change message) → `ToolFailure[T_tool]` warning that feedback was not responded to, directing the agent to change, blame, fail, or advance again if no changes are needed.
