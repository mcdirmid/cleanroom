...
# Implementation LLS: build_node_loader_impl

## Data Types
```python
from build_node_loader import BuildNodeLoader, BuildNode
from tool_provider import ToolProvider, ToolExecutor
from agent_loop import AgentResult

class BuildNodeImpl(BuildNode): ...

class BuildNodeLoaderImpl(BuildNodeLoader): ...
```

`BuildNodeImpl` extends the interface's `BuildNode` data-class Protocol (which itself extends `ToolProvider`), adding implementation-internal state and implementing the tool-provider operations:
...
- `_dependency_nodes`: list of `BuildNodeImpl` instances for each dependency, including silent dependencies (resolved at load time).
- `_agent_loop`: the agent loop used for running prompts (set externally after construction).

**Manifest fields** (static data from manifest, per the interface spec's `BuildNode`):
- `label`, `prompt`, `tools`, `deps`, `silent_deps`, `feedback_deps`, `src`, `template`, `guide`, `silent_srcs`.
...
Constructed with no configuration (`BuildNode` instances are constructed by the implementation); the agent loop used for prompt running is set externally on each node after construction (see Internal state).

## Behavioral Description

`BuildNodeLoaderImpl` (declared above) implements the `BuildNodeLoader` Protocol by constructing nodes from manifests and caching them by label.

- **`load_node`** — Loads a node from its manifest file (detected via runfiles). Constructs a `BuildNodeImpl` with all manifest fields; tool providers are resolved lazily on first use and cached in `_tool_providers`. Caches the result by label on success. Returns `None` if the manifest does not exist. A manifest that cannot be deserialized raises an error; the cache is not populated (a node is cached only after successful deserialization).

- **`load_graph`** — Loads the root node and recursively loads its dependencies (declared and silent). Returns a mapping of labels to `BuildNodeImpl` instances (root and all transitive deps, including silent deps).
