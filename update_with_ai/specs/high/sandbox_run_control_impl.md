...
The advance tool is named `advance` and accepts a text *change summary* parameter that must be omitted while guide steps remain and is required only when concluding the session after modifying workspace files. Responses from the advance tool share a constant suppression key `advance`. Executing the advance tool coordinates self-contained advancement:
